...
if [ ! -f "$APK_SOURCE" ]; then
    ...
    (cd android && JAVA_HOME=$HOME/android-sdk-setup/jdk ANDROID_HOME=$HOME/android-sdk-setup/android-sdk PATH=$HOME/android-sdk-setup/jdk/bin:$PATH ./gradlew assembleDebug)
fi

echo "✔ Found compiled APK. Packaging to ZIP (to bypass Firebase Spark Billing Plan executable restrictions)..."
rm -f frontend/caledonian-mercury.apk
